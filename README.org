...
*** "Use" statements
	...
	You can embed "use" statements to do migrations on a number of different
	databases (in MySQL, "schemas" in Postgres, etc.) if you add a parameter:
	
     : (def config {
	 :         ;; .....
     :              :migration-database-name        "migrations/"
